...
exports.run = async (client, message, args) => {
    ...
    const url = `https://www.futbin.com/consumables/Position%20Change`;

    let htmlData = await getHtmlData(url);

    const title = `Prices position consumables items`;
    const description = `This is a list of all 20 position consumables with their prices.`;
    const author = `All prices of the position consumables items`;
